        ...
        <footer className="bg-gradient-to-br from-slate-50 to-blue-50 text-slate-700">
            ...
            <div className="fixed right-3 bottom-4 sm:right-4 sm:bottom-6 md:right-6 md:bottom-8 flex flex-col items-end gap-3 sm:gap-4 z-50">
                ...
                <button
                    ...
                    className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-3 py-2 sm:px-4 sm:py-3 rounded-full shadow-2xl flex items-center gap-2 sm:gap-3 hover:from-blue-600 hover:to-blue-700 transform transition-all duration-200 hover:-translate-y-0.5 focus:outline-none focus:ring-4 focus:ring-blue-200"
                >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                    <div className="bg-white rounded-xl w-full max-w-md p-4 sm:p-6 shadow-xl">
                        <div className="flex items-center justify-between mb-3 sm:mb-4">
                            <h3 className="text-base sm:text-lg font-semibold">Leave a Testimonial</h3>
                            <button onClick={() => setShowTestimonial(false)} className="text-slate-500 hover:text-slate-800 text-xl sm:text-2xl">✕</button>
                        </div>
                        <form onSubmit={handleTestimonialSubmit} className="space-y-2 sm:space-y-3">
                            <input required name="name" ref={nameRef} placeholder="Your name" className="w-full border rounded-md px-3 py-2 text-sm sm:text-base" />
                            <input name="board" placeholder="Board (CBSE/ICSE)" className="w-full border rounded-md px-3 py-2 text-sm sm:text-base" />
                            <input name="grade" placeholder="Grade / Class" className="w-full border rounded-md px-3 py-2 text-sm sm:text-base" />
                            <input name="country" placeholder="Country" className="w-full border rounded-md px-3 py-2 text-sm sm:text-base" />
                            <textarea name="content" value={testimonialContent} onChange={e => setTestimonialContent(e.target.value.slice(0,200))} placeholder="Your testimonial (max 200 chars)" className="w-full border rounded-md px-3 py-2 text-sm sm:text-base" rows={4} />
                            <div className="flex items-center justify-between text-xs text-slate-500">
                                <span>{testimonialContent.length}/200</span>
                                <span className="text-red-500">{errorMessage}</span>
                            </div>
                            <div className="flex items-center gap-2 sm:gap-3 justify-end">
                                <button type="button" onClick={() => setShowTestimonial(false)} className="px-3 sm:px-4 py-1.5 sm:py-2 text-sm sm:text-base rounded-md border">Cancel</button>
                                <button type="submit" disabled={submitting} className="px-3 sm:px-4 py-1.5 sm:py-2 text-sm sm:text-base bg-blue-600 text-white rounded-md disabled:opacity-60">{submitting ? 'Submitting...' : 'Submit'}</button>
                            </div>
                        </form>
                    </div>
                </div>
